...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream.flatMap(customer -> customer.getWantToBuy().stream()).map(Item::getName)
                .filter(item -> itemListOnSale.stream().noneMatch(item::equals)).collect(Collectors.toSet());
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = customer -> customer.getBudget() >= customer.getWantToBuy().stream().mapToInt(item -> onSale.stream()
                .filter(shopItem -> shopItem.getName().equals(item.getName())).sorted((o, o1) -> o.getPrice() - o1.getPrice()).findFirst().map(Item::getPrice).orElse(0)).sum();
